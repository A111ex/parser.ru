 ...
 * @since 2.0
 */
class MyActionColumn extends \yii\grid\Column
{
    /**
     ...
     * @see buttons
     */
    public $template = '{params} {update} {delete}';
    /**
    ...
    {
        if (!isset($this->buttons['params'])) {
            $this->buttons['params'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => 'Параметры',
                    'aria-label' => Yii::t('yii', 'View'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
                return Html::a('<span class="glyphicon glyphicon-eye-open"></span>', ['/goods-params-name', 'goodType' => $model->type], $options);
            };
        }
